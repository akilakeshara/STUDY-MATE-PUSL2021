...
class StudentCommunityChatScreen extends StatefulWidget {
  const StudentCommunityChatScreen({super.key});

  @override
  State<StudentCommunityChatScreen> createState() =>
      _StudentCommunityChatScreenState();
}

class _StudentCommunityChatScreenState
    extends State<StudentCommunityChatScreen> {
  ...
  void _sendMessage() async {
    ...
    if (_editingDocId != null) {
      ...
      await FirebaseFirestore.instance.collection('community_messages').add(docData);
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        Text("Start the conversation!", style: TextStyle(color: Colors.grey.withOpacity(0.5))),
                      ],
                    ),
                  );
                }
  ...
  Widget _buildMessageBubble(
    ...
    final String senderName = data['senderName'] ?? "Student";
    const primaryColor = Color(0xFF5C71D1);
  ...
  Widget _buildMessageActions(String docId, Map<String, dynamic> data, bool isMe, ThemeData theme) {
    return PopupMenuButton<String>(
      ...
      onSelected: (value) {
        if (value == 'reply') {
          _startReplying(docId, data['text'] ?? "", data['senderName'] ?? "Student");
        } else if (value == 'edit') {
              ...
              Expanded(
                child: Container(
                  ...
                  child: Row(
                    children: [
                      const IconButton(
                        icon: Icon(Icons.emoji_emotions_outlined, color: Colors.grey, size: 22),
                        onPressed: null,
                      ),
                      Expanded(
                        child: TextField(
